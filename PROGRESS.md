...
> 更远期(设计文档 §10):窗口/应用焦点检测(需 `active-win` 类原生依赖或 PowerShell 前台窗口查询 + 应用名→台词映射配置,复杂度明显更高,已拆到独立后续轮次)、主动搭话/陪伴深化("性格 SOUL",依赖情境感知信号先落地,已拆到独立后续轮次)、配音播放(lines.json 的 audio 字段已读入但未播放)、情绪/养成数值系统。
