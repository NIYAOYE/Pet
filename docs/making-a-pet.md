...
**当前真正会被触发的分类只有这 9 个 + 下面单独介绍的 `app_focus`**（对应 `src/shared/reactionPlanner.ts` 的 `ReactionCategory`，写其他分类名不会报错，但也永远不会被读到）：
...
### 额外分类：`app_focus`（切到某个应用时吐槽一句）

`app_focus` 是 `lines.json` 里唯一一个**不是**扁平数组的分类——因为它需要先按规则匹配"当前切到了哪个应用"，再从命中规则自己的台词池里选一句，格式是一个**规则数组**：
...
- `match`：字符串数组，对当前前台窗口的**进程名 + 窗口标题**拼在一起做**不区分大小写的子串匹配**，命中其中任意一条就算这条规则命中。
- 多条规则按数组顺序匹配，取**第一个**命中的；命中后从它的 `lines` 池里随机选一句（同样会避免紧接着复读上一句）。
- 检测每 3 秒轮询一次前台窗口（Windows-only，PowerShell 查询，零新依赖），带两层防刷屏冷却：任意两次吐槽间隔至少 20 秒，同一条规则命中后 15 分钟内不会再触发一次。
- **不写 `app_focus` 这个分类，这只宠物就完全不会检测前台窗口**——不是"检测了但恰好不吐槽"，而是背后的轮询循环根本不会启动，零后台开销、零隐私顾虑。
