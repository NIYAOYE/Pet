...
```ts
  ...
  ipcMain.on(IPC.GENIE_START_INSTALL, () => {
    ...
    void runGenieRuntimeInstall({
      destDir,
      steps: {
        downloadEmbeddablePython: (dir) => realDownloadEmbeddablePython(dir, PYTHON_EMBED_DOWNLOAD_URL),
        enablePip: async (dir, onProgress) => {
          await installWithMirrorFallback({
            candidates: PIP_MIRROR_CANDIDATES,
            install: (candidate: MirrorCandidate) => realPipInstall(dir, ['--upgrade', 'pip'], {
              indexUrl: candidate.indexUrl, fastFail: candidate.fastFail, onOutput: onProgress
            })
          })
        },
        installGenieTts: async (dir, onProgress) => {
          await installWithMirrorFallback({
            candidates: PIP_MIRROR_CANDIDATES,
            install: (candidate: MirrorCandidate) => realPipInstall(dir, ['genie-tts'], {
              indexUrl: candidate.indexUrl, fastFail: candidate.fastFail, onOutput: onProgress
            })
          })
        },
      ...
      onProgress: (p) => petWin.webContents.send(IPC.GENIE_INSTALL_PROGRESS, p)
    }).then((r) => {
      if (r.ok) {
...
`PYTHON_EMBED_DOWNLOAD_URL`/`PIP_MIRROR_CANDIDATES`/`installWithMirrorFallback`/`MirrorCandidate` 这些
名字要跟现有 GSV 那段实际使用的变量名对齐——**执行本任务时先读一遍 GSV 安装 handler
(`ipcMain.on(IPC.VOICE_START_INSTALL, ...)`,在 `IPC.VOICE_GET_STATE` handler 之后几十行)的现有代码,
把这几个名字替换成那里实际用的名字**,不要凭本计划猜的名字硬编,以免和现有变量名对不上导致编译错误。
...
**执行本任务时**,先找到现有代码里调用 `currentTts()`(保存设置时组装 `AppSettings` 的地方)和
`applyTts(...)`(载入设置时回填表单的地方)的具体位置,以及第 386 行
`ttsRuntimeStatus.textContent = formatRuntimeState(await window.voiceApi.getState())` 所在的状态
刷新函数——在这三处旁边分别加上对称的 `ttsGenie: currentTtsGenie()` / `applyTtsGenie(loaded.ttsGenie)`
/ `genieRuntimeStatus.textContent = formatGenieRuntimeState(await window.genieVoiceApi.getState())`。
这三处的具体上下文本计划未逐行摘录,执行者需要打开文件实地看一遍再改,保持和 `tts` 字段的现有处理
方式完全对称。
