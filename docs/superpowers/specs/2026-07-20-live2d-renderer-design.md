...
| 场景 | 目标帧率 |
|---|---:|
| 活动（拖拽、行走、说话、动作播放）、普通待机、睡眠 | 跟随显示器刷新率（不限速） |
| 隐藏、最小化、锁屏 | 0 FPS |

> 2026-07-22 修订（Phase 7）：原表格曾对“普通待机”“睡眠”分别设 30/15 FPS 降帧
> 目标。真机验收（Phase 6）反馈这套降帧没有意义——桌宠本来资源占用就不高，
> 不需要为了省那点占用换取观感上的降帧（降帧在待机/睡眠时反而更容易被注意到
> 卡顿感）——已在 `d5d2785` 整体移除 `fpsForState()`/`live2dFps.ts` 模块和
> `playState()` 里对 `ticker.maxFPS` 的赋值，待机/睡眠不再单独限速，与活动
> 状态一样跟随 Pixi Ticker 默认（`maxFPS=0`，即跟随显示器刷新率）。隐藏/
> 最小化/锁屏仍然显式停止 Ticker（见下方“具体措施”），这条不受影响。
...
| 指标 | 目标 |
|---|---|
| 活动动画（含普通待机、睡眠，见 §10 2026-07-22 修订） | 目标 60 FPS；连续 5 分钟掉帧率低于 5% |
