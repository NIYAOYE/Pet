...
| `petController`（扩展） | `src/renderer/petController.ts` | 订阅新 IPC 信号，转成 `reactionPlanner` 的新触发类型；`break_reminder` 若要求叫醒，向 `petBrain` 注入 `wake` `PetEvent`（复用现有 `send()`/`IPC.PET_EVENT` 通道） | 现有手势层 |

**数据流**：`idleWatcher`（主进程，每 30s 采样）→ 边沿检测出 `afk_leave`/`break_reminder` → `IPC.CONTEXT_SIGNAL` 推给渲染进程 → `petController` 存入待处理队列 → 下一 tick 喂给 `stepReaction` → 若 `output.speak` → `petApi.petSpeak(category)`（复用 MVP-13 既有链路，不变）；若 `output.wake` → `petController.send('wake')`（复用 MVP-10 既有 `PetEvent` 处理链路，不变）。
...
- `ReactionOutput`：`{ speak?: ReactionCategory }` → `{ speak?: ReactionCategory; wake?: boolean }`（`wake: true` 告诉调用方要把宠物从睡眠中叫醒）。
...
3. `trigger === 'break_reminder'` → 说 `break`；`output.wake = sleeping`（若采样时宠物正在睡则要求调用方叫醒它）。
...
4. `src/renderer/petController.ts`（或 `main.ts` 初始化处，仿 `onPetEvent` 的接线位置）：订阅 → 存入**独立于 `pendingReaction` 的新队列字段**（如 `pendingContextSignal`）→ 下一 tick 喂给 `stepReaction`；`output.wake` 为真时调用 `this.send('wake')` 把 `PetEvent` 排进 `pending`（下一 tick 内、约 33ms 后由 `petBrain` 实际处理，视觉上感知不到延迟）。

**时序陷阱与修正**：`tick()` 里现有的 `wokeUp = prevState==='sleep' && ctx.state!=='sleep'` 检测，与 `break_reminder` 自己注入的 `wake` `PetEvent` 会在**同一 tick** 内都判定为真——若不处理，`trigger` 会被 `wokeUp` 派生成通用 `wake`，盖掉本该更具体的 `break_reminder`，`break` 台词永远出不来。修正：`tick()` 里 trigger 的优先级改为 `pendingContextSignal ?? (wokeUp ? 'wake' : (pendingReaction ?? undefined))`——**新的 `pendingContextSignal` 队列优先于 `wokeUp` 派生**，但 `wokeUp` 相对于既有 `pendingReaction`（poke/拖拽）的优先级**保持不变**，不影响 MVP-13 已验收的"拖起睡眠中的宠物出 `wake` 而非 `drag`"手感。

## 8. 测试策略

- **`reactionPlanner`**（Vitest 纯函数，扩展现有测试文件）：新增用例覆盖睡眠戳→`sleep`、当天首问候的早安/晚安窗口判定与跨天重置、`afk_leave`→`farewell`（睡眠中也触发）、`break_reminder`→`break`+`wake`标记（睡眠中触发要求叫醒，非睡眠中不要求）、`pausedByDialog` 依旧对新信号生效。日期/时段判定需要注入固定 `nowMs` 做判别性测试。
...
- `src/renderer/petController.ts`（订阅 `onContextSignal`、喂新触发类型、`output.wake` 时注入 `PetEvent`、`stepReaction` 调用点传入 `sleeping`/`pausedByDialog`/`nowMs`）
